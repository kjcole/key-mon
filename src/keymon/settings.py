...
class CommonFrame(Gtk.Frame):
    ...
    def _add_dropdown(self, vbox, title, tooltip, opt_lst,
                      option, width_char=-1):
        """Add a drop down box."""
        hbox = Gtk.Box()
...
class MiscFrame(CommonFrame):
    ...
    def create_layout(self):
        ...
        self._add_dropdown(
            vbox,
            _("Scale:"),
            _("How much larger or smaller than normal to "
              "make key-mon. Where 1.0 is normal sized."),
            sizes,
            "scale",
            4,
        )
        ...
        self.kbd_files = sorted(
            list(set(os.path.basename(kbd)
                     for kbd in self.settings.options.kbd_files))
        )
        self._add_dropdown(
            vbox,
            _("Keymap:"),
