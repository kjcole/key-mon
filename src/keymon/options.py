...
class OptionItem(object):
  ...
  """
    ...
    def add_to_parser(self, parser):
        ...
        parser.add_option(dest=self._dest,
                          type=self._type,
    ...
    def _add_bool_to_parser(self, parser):
        ...
        if self._ini_group:
            ...
            parser.add_option("--no" + self._opt_long.lstrip("-"),
                              action="store_false",
                              dest=self._dest,
                              help=_(f"Opposite of {self._opt_long}"))
...
class Options(object):
    ...
    """
        ...
        self._options[dest] = OptionItem(dest, type, default, name, help,
    ...
    def parse_ini(self, fp):
        ...
        for opt in list(self._options.values()):
            if opt.ini_group:
                ...
                if config.has_section(opt.ini_group) and config.has_option(
                    opt.ini_group, opt.ini_name
                ):
                    opt.value = config.get(opt.ini_group, opt.ini_name)
                    LOG.info(
                        f"From ini getting {opt.ini_group}.{opt.ini_name} = {opt.value}"
                    )
...
if __name__ == "__main__":
    o = Options()
    o.add_option(opt_short="-l",
                 opt_long="--larger",
                 dest="larger",
                 default=False,
                 type="bool",
                 help="Make the dialog 25% larger than normal.")
    o.add_option(opt_short="-m",
                 opt_long="--meta",
                 dest="meta",
                 type="bool",
                 ini_group="buttons",
    ...
    o.add_option(opt_short="-v",
                 opt_long="--version",
                 dest="version",
                 type="bool",
                 help="Show version information and exit.")
    o.add_option(opt_short="-t",
                 opt_long="--theme",
                 dest="theme",
                 type="str",
                 ini_group="ui",
    ...
    o.add_option(opt_long="--old-keys",
                 dest="old_keys",
                 type="int",
                 ini_group="buttons",
                 ini_name="old-keys",
