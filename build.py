...
def build_screen_shots():
    ...
    todos = [
        ("screenshot", [],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-blank", [],
         ["KEY_EMPTY"]),
        ("screenshot-smaller", ["--smaller"],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-larger", ["--larger"],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-apple", ["--theme", "apple"],
         all_buttons + ["BTN_RIGHT"]),
        ("screenshot-oblivion", ["--theme", "oblivion"],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-modern", ["--theme", "modern"],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-big-letters", ["--theme", "big-letters"],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-clear", ["--theme", "clear"],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-clear-command", ["--theme", "clear-command"],
         all_buttons + ["BTN_LEFT"]),
        ("screenshot-clear-ubuntu", ["--theme", "clear-ubuntu"],
         all_buttons + ["BTN_LEFT"]),
        ("2x-no-mouse-meta", ["--nomouse", "--scale", "2.0", "--meta"],
         all_buttons + ["KEY_SUPER_L"]),
        ("old-keys-2", ["--noctrl", "--noalt", "--nomouse", "--old-keys", "2"],
         ["KEY_Y", "KEY_Y", "KEY_P"]),
    ]
    for fname, options, keys in todos:
        pybdist.clean_config(setup)
        subprocess.call(["python", prog] + options +
                        ["--screenshot", ",".join(keys)])
...
def main():
    ...
    parser.add_option(
        "--png", dest="png", action="store_true", help="Only build png files"
    )
